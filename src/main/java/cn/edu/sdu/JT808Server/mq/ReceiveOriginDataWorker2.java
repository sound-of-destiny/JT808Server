...
public class ReceiveOriginDataWorker2 implements Runnable {
    ...
    public void run() {
        try {
            ...
            System.out.println(" [*] Origin Worker2 正在等待消息");

            DeliverCallback deliverCallback = (consumerTag, delivery) -> {
                try {
                    File data = new File("./JT808ServerOriginData/" + LocalDate.now() + "-2.bytes");
